  ...
  $.getJSON(url, null, function (data, status) {
    if (status === "success"){ // si la requête se déroule bien
      ...
      $(".table-row").each(function(i) {
        $(".table-data:eq(1)", this).text(shortNbr(data[i].current_price) + "€")
        $(".table-data:eq(2)", this).text(shortNbr(data[i].market_cap) + "€")
        $(".table-data:eq(3)", this).text(shortNbr(data[i].total_volume) + "€")
        $(".table-data:eq(4)", this).text(shortNbr(data[i].total_supply) + "€")
        $(".table-data:eq(5)", this).text(data[i].price_change_percentage_24h + "%")
      })
...
function shortNbr (labelValue) {
  ...
  if (val >= 1.0e+9) {
    val = parseInt(val / 1.0e+9) + "B"
  } else if (val >= 1.0e+6) {  
    val = parseInt(val / 1.0e+6) + "M"
  }
